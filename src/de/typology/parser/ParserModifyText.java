...
public class ParserModifyText {
    ...
    /**
     * Removes xml childs or other stuff from a text. Method is in bad complexity class and runtime can be improved by alot
     * @param text - in which to search for
     * @param open - tag that opens e.g. <ref>
     * @param close - tag that closes e.g. </ref>
		...
		for (int i=0;i<text.length();i++){
    		...
    		if (substr.startsWith(close)){
    			...
    			if (stack.isEmpty()){
    				...
    				String end = text.substring(i+close.length(),text.length());
    				text = begin + end; 
    				i = startPos;
    			}
    		}
