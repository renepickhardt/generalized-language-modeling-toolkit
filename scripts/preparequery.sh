...
#query="select source, target, score from ${fEdge} where ${fEdge}.source like \"${fSource}\" AND ${fEdge}.target like \"${prefix}\" order by score desc limit 5; 
#select source, target, score from ${tEdge} where ${tEdge}.source like \"${tSource}\" AND ${tEdge}.target like \"${prefix}\" order by score desc limit 5; 
#select source, target, score from ${sEdge} where ${sEdge}.source like \"${sSource}\" AND ${sEdge}.target like \"${prefix}\" order by score desc limit 5; 
#select source, target, score from ${oEdge} where ${oEdge}.source like \"${oSource}\" AND ${oEdge}.target like \"${prefix}\" order by score desc limit 5;"
